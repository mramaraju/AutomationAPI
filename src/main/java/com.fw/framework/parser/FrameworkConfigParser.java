...
public class FrameworkConfigParser {
	...
	private String Organisation_Email = "framework@gmail.com";
	private String Organisation_Website = "www.gmail.com";
	...
	private void addFeatureList(Document doc, Element rootElement) {
		...
		{
			Element property = doc.createElement("property");
			property.appendChild(doc.createTextNode(Boolean.toString(isenableDebug())));
			features.appendChild(property);

			Attr attr = doc.createAttribute("name");
			attr.setValue("enableDebug");
			property.setAttributeNode(attr);
		}
		...
		for (int temp = 0; temp < nList.getLength(); temp++) {
			...
			for (int i = 0; i < nChildList.getLength(); i++) {
				...
				if (nChildNode.getNodeType() == Node.ELEMENT_NODE) {
					...
					} else if ("enableDebug".equals(eElement.getAttribute("name"))) {
						setenableDebug(Boolean.parseBoolean(eElement.getTextContent()));
