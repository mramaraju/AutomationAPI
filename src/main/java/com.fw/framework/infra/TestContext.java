...
import com.fw.framework.SystemProperties;
...
public class TestContext {
	...
	public void setTestStatus(TestStatus testStatus, File snapshot, String description) {
		...
		if (testStatus.getValue() >= currentTestStatus.getValue()) {
			...
			if (testStatus == TestStatus.FAIL) {
				//@formatter:off
				getLogger().info(FWStaticStore.FW_TEST_FAIL_STAMP);
				//@formatter:on
			}
		}
	}
	...
	protected void generateTestSummary(TestObjectWrapper t) {
		...
		if (isKnownToFail()) {
			if (getCurrentTestStatus() == TestStatus.PASS) {
				//@formatter:off
				getLogger().info(FWStaticStore.FW_KTF_TEST_PASSED_STAMP);
				//@formatter:on
	...
	protected void generateUnitTestSummary(TestUnitObjectWrapper unit) {
		...
		if (unit.isKTF()) {
			if (getCurrentUnitTestStatus() == TestStatus.PASS) {
				//@formatter:off
				getLogger().info(FWStaticStore.FW_KTF_TESTUNIT_PASSED_STAMP);
				//@formatter:on
	...
	private void printUsefulInfo() {
		...
		sb.append("\n");
		//sb.append("* version => " + FWStaticStore.FW_BUILD_VERSION);
		sb.append("\n");
		//sb.append("* build date => " + FWStaticStore.FW_BUILD_DATE);
		sb.append("\n");
